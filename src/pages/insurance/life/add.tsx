...
export default function AddLifeInsurance() {
  ...
  const [riders, setRiders] = useState([{ id: 1 }]);
  const [nominees, setNominees] = useState([{ id: 1 }]);
  const [documents, setDocuments] = useState([{ id: 1 }]);

  const addRider = () => setRiders([...riders, { id: Date.now() }]);
  const removeRider = (id: number) => setRiders(riders.filter(r => r.id !== id));

  const addNominee = () => setNominees([...nominees, { id: Date.now() }]);
  const removeNominee = (id: number) => setNominees(nominees.filter(n => n.id !== id));
      ...
      <div className="w-full mx-auto animate-in fade-in duration-500 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="sticky top-0 z-40 bg-white flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-gray-200 pb-5 mb-8 pt-6 -mt-6 -mx-6 px-6 rounded-t-xl">
          <div className="flex items-center gap-3 font-bold text-gray-900">
            ...
            <h1 className="text-xl">Add Life Insurance</h1>
          </div>
          ...
          <form className="space-y-8 bg-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-5">
                <div>
                  ...
                  <Select className={inputClass}>
                    ...
                    <option value="2">Rohan Sharma</option>
                    <option value="3">Priya Singh</option>
                    <option value="4">Neha Gupta</option>
                    <option value="5">Vikram Verma</option>
                  </Select>
                </div>
                <div>
                ...
                <div>
                  ...
                  <Select className={inputClass}>
                    ...
                    <option value="1">Rahul Desai</option>
                    <option value="2">Amit Shah</option>
                    <option value="3">Suresh Patel</option>
                    <option value="4">Anjali Mehta</option>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-5">
                ...
                <div>
                  ...
                  <Select className={inputClass}><option>Select Payment Mode</option></Select>
                </div>
                <div>
                ...
                <div>
                  ...
                  <DatePicker className={inputClass} placeholder="Select Login Date" />
                </div>
                <div>
                  <label className={labelClass}>Policy Start Date <span className="text-red-500">*</span></label>
                  <DatePicker className={inputClass} placeholder="Select Start Date" />
                </div>
                <div className="lg:col-span-2">
                  <label className={labelClass}>Policy Premium End Date <span className="text-red-500">*</span></label>
                  <DatePicker className={inputClass} placeholder="Select Premium End Date" />
                </div>

                <div>
                  <label className={labelClass}>Policy Maturity Date <span className="text-red-500">*</span></label>
                  <DatePicker className={inputClass} placeholder="Select Maturity Date" />
                </div>
                <div>
                  <label className={labelClass}>Maturity Amount</label>
                  <input type="text" placeholder="Enter Maturity Amount" className={inputClass} />
                </div>
                <div className="lg:col-span-2">
                  ...
                  {riders.map((rider, index) => (
                    <div key={rider.id} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-4 items-end">
                      <div>
                        ...
                        <Select className={inputClass}><option>Select Rider</option></Select>
                      </div>
                      <div>
                      ...
                      <div className="lg:col-span-2 flex items-center gap-4">
                        <div className="flex-1">
                          ...
                          <input type="text" placeholder="Enter Note" className={inputClass} />
                        </div>
                        {index === 0 ? (
                ...
                <div>
                  <label className={labelClass}>Total Net Premium<span className="text-red-500">*</span></label>
                  <input type="text" defaultValue="0" className={inputClass} />
                </div>
                <div>
                  <label className={labelClass}>GST Amount</label>
                  <input type="text" placeholder="Enter GST Amount" className={inputClass} />
                </div>
                <div className="lg:col-span-2">
            ...
            <div>
              ...
              <div className="flex items-start gap-4 py-2 px-1">
                <input type="checkbox" className="mt-1 w-4 h-4 border-gray-300 rounded text-[#2B4399] focus:ring-[#2D3591]" />
                <div>
                ...
                {nominees.map((nominee, index) => (
                  <div key={nominee.id} className="flex items-center gap-4">
                    <div className="flex-[2]">
                      <input type="text" placeholder="Nominee Name *" className={inputClass} />
                    </div>
                    <div className="flex-[2]">
                      <Select className={inputClass}><option>Select Relationship</option></Select>
                    </div>
                    <div className="flex-1">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-5">
                ...
                <div>
                  ...
                  <input type="text" placeholder="Enter Account Type" className={inputClass} />
                </div>
                <div>
                  <label className={labelClass}>Account Number</label>
                  <input type="text" placeholder="Enter Account Number" className={inputClass} />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div>
                  <label className={labelClass}>IFSC CODE</label>
                  <input type="text" placeholder="Enter IFSC CODE" className={inputClass} />
                </div>
                <div>
                  <label className={labelClass}>Account Holder Name</label>
                  <input type="text" placeholder="Enter Account Holder Name" className={inputClass} />
                </div>
              </div>
            </div>
