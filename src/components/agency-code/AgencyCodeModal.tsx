...
export default function AgencyCodeModal({
  ...
  const handleChange = (field: keyof AgencyCodeFormData, value: string) => {
    ...
    if (errors[field]) {
      const { errors: newErrors } = validateAgencyCode(updatedForm);
      setErrors(prev => ({ ...prev, [field]: newErrors[field] || '' }));
    }
  };

  const handleBlur = (field: keyof AgencyCodeFormData) => {
    if (errors[field] || formData[field]) {
      const { errors: newErrors } = validateAgencyCode(formData);
      setErrors(prev => ({ ...prev, [field]: newErrors[field] || '' }));
    }
  };
