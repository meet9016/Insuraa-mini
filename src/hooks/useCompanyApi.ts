...
export const useCompanyActions = () => {
  ...
  const insertCompany = async (name: string, editingCompanyId?: string | number | null) => {
    ...
    try {
      ...
      if (editingCompanyId) {
        payload.company_id = String(editingCompanyId);
      }
      const response = await api.post(endPointApi.COMPANY.INSERT_COMPANY, payload);
      resData = response.data;
    }

    if (resData?.status === 200 || resData?.status === 'success' || resData?.status === true) {
      toast.success(resData?.message || (editingCompanyId ? 'Company updated successfully!' : 'Company saved successfully!'));
